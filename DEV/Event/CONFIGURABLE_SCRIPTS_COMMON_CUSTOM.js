...
function getContactsFromSessionForASB() {
	...
	if (contactModelList != "" && contactModelList != null) {
		...
		for (var i = 0; i < contactModelList.size(); i++) {
			...
			peopleTemplate = contactModelList.get(i).getPeople().getTemplate();
			if (peopleTemplate) {
				...
				if (!(templateGroups == null || templateGroups.size() == 0)) {
					...
					for (var subGroupIndex = 0; subGroupIndex < subGroups.size(); subGroupIndex++) {
						...
						for (var fieldIndex = 0; fieldIndex < fields.size(); fieldIndex++) {
							...
							contact["asi." + field.getDisplayFieldName()] = field.getChecklistComment();		
						}
					}
				}
			}
...
function getContactsFromSessionAV() {
	...
	if (contactModelList != "" && contactModelList != null) {
		...
		for (att in atts)
			contact[atts[att].attributeName] = atts[att].attributeValue;
		peopleTemplate = contactModel.getPeople().getTemplate();
		if (peopleTemplate != null) {
			var templateGroups = peopleTemplate.getTemplateForms(); //ArrayList
			if (!(templateGroups == null || templateGroups.size() == 0)) {
				thisGroup = templateGroups.get(0);
...
function getContactsFromSession4ACA() {
	...
	if (r.size() > 0) {
		for (var cc = 0; cc < r.size(); cc++) {
			...
			for (att in atts)
				contact[atts[att].attributeName] = atts[att].attributeValue;
			peopleTemplate = r.get(cc).getPeople().getTemplate();
			if (peopleTemplate != null) {
				var templateGroups = peopleTemplate.getTemplateForms(); //ArrayList
				if (!(templateGroups == null || templateGroups.size() == 0)) {
					thisGroup = templateGroups.get(0);
...
function getContactsList() {
	...
	if (contactArray.length > 0) {
		for (var cc = 0; cc < contactArray.length; cc++) {
			...
			for (att in atts)
				contact[atts[att].attributeName] = atts[att].attributeValue;
			peopleTemplate = contactArray[cc].getPeople().getTemplate();
			if (peopleTemplate != null) {
				var templateGroups = peopleTemplate.getTemplateForms(); //ArrayList
				if (!(templateGroups == null || templateGroups.size() == 0)) {
					thisGroup = templateGroups.get(0);
					...
					for (var subGroupIndex = 0; subGroupIndex < subGroups.size(); subGroupIndex++) {
						...
						for (var fieldIndex = 0; fieldIndex < fields.size(); fieldIndex++) {
							var field = fields.get(fieldIndex);
							contact["asi." + field.getDisplayFieldName()] = field.getChecklistComment();		
							logDebug("asi." + field.getDisplayFieldName() + "=" + field.getChecklistComment());			
						}
					}
				}
			}
...
function isContactMatchRules(contactFieldsJson) {
	...
	for (ca in contactsArray) {
		...
		for ( var ct in contactFieldsJson) {
			if (ct == "asi.Tax Exempt") {
				if (contactsArray[ca]["contactType"].equals("Business Entity") || contactsArray[ca]["contactType"].equals("Sponsoring Organization")) {
						rowMatched = rowMatched && contactFieldsJson[ct].equals(contactsArray[ca][ct]);
				}
				else rowMatched = false;
			}
			else {
				rowMatched = rowMatched && contactFieldsJson[ct].equals(contactsArray[ca][ct]);
			}
