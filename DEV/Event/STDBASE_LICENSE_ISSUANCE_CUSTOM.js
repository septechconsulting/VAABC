/*==========================================================================================
Title : STDBASE_LICENSE_ISSUANCE_CUSTOM
...
var scriptSuffix = "LICENSE_ISSUANCE_CUSTOM";
...
try {
	...
	if (isConfigurableScript(settingsArray, scriptSuffix)) {
		for (s in settingsArray) {
			...
			if (appTypeArray[2] == "Internet Retailer")
				licenseIssuanceRetailer(capId, rules.action);
			else
 ...
 */
function licenseIssuanceRetailer(itemCapId, recordSettings) {
	...
	var rHasParent, rLicAppArray, rNewWineLicId, rNewWineLicIdString, rNEwBeerLicId, rNewBeerLicIdString, rVehData, rChildVehId, rC1ExpResult, rB1Model;
	...
	if (rParentLicense != "") {
		...
		//create license
		wineChkBox = getAppSpecific("Wine", capId);
		beerChkBox = getAppSpecific("Beer", capId);
		locField = getAppSpecific("Location of business", capId);
		if (locField == "In state") locField = "In State";
		if (locField == "Out of state") locField = "Out of State";
		if (wineChkBox != "CHECKED" && beerChkBox != "CHECKED")
			return;
		
		if (wineChkBox == "CHECKED")
			rNewWineLicId = createParentLocal(rLicAppArray[0], rLicAppArray[1], rLicAppArray[2], rLicAppArray[3], null);
		if (beerChkBox == "CHECKED")
			rNewBeerLicId = createParentLocal(rLicAppArray[0], rLicAppArray[1], rLicAppArray[2], rLicAppArray[3], null);

		if (!rNewWineLicId && !rNewBeerLicId) {
		...
		if (rNewWineLicId) {
			var newWineCap = aa.cap.getCap(rNewWineLicId).getOutput();
			capIdsArray.push(rNewWineLicId);
			rNewWineLicIdString = rNewWineLicId.getCustomID();
		}
		if (rNewBeerLicId) {
			var newBeerCap = aa.cap.getCap(rNewBeerLicId).getOutput();
			capIdsArray.push(rNewBeerLicId);
			rNewBeerLicIdString = rNewBeerLicId.getCustomID();
		}
		

		if (rIssuedStatus != null && rIssuedStatus != "") {
			if (rNewWineLicId) updateAppStatus(rIssuedStatus, "", rNewWineLicId);
			if (rNewBeerLicId) updateAppStatus(rIssuedStatus, "", rNewWineLicId);
		}
		...
		if (!isEmptyOrNull(recordSettings.copyCustomFields)) {
			...
			if (rNewWineLicId) {
				editAppSpecific("Beer", "UNCHECKED", rNewWineLicId);
			}
			if (rNewBeerLicId) {
				editAppSpecific("Wine", "UNCHECKED", rNewBeerLicId);
			}
		}
		...
		if (recordSettings.copyEducation) {
			if (rNewWineLicId) aa.education.copyEducationList(itemCapId, rNewWineLicId);
			if (rNewBeerLicId) aa.education.copyEducationList(itemCapId, rNewBeerLicId);
		}

		if (recordSettings.copyContinuingEducation) {
			if (rNewWineLicId) aa.continuingEducation.copyContEducationList(itemCapId, rNewWineLicId);
			if (rNewBeerLicId) aa.continuingEducation.copyContEducationList(itemCapId, rNewBeerLicId);
		}

		if (recordSettings.copyExamination) {
			if (rNewWineLicId) aa.examination.copyExaminationList(itemCapId, rNewWineLicId);
			if (rNewBeerLicId) aa.examination.copyExaminationList(itemCapId, rNewBeerLicId);
		}
		...
		//handle Expiration	
		if (rNewWineLicId) {
			rB1ExpResult = aa.expiration.getLicensesByCapID(rNewWineLicId).getOutput();
			if (rB1ExpResult != null && rB1ExpResult.getB1Expiration() != null) {
				...
				logDebug("**WARN rB1ExpResult is null for created Parent License " + rNewWineLicId);
			}
		}
		if (rNewBeerLicId) {
			rB1ExpResult = aa.expiration.getLicensesByCapID(rNewWineLicId).getOutput();
			if (rB1ExpResult != null && rB1ExpResult.getB1Expiration() != null) {
				...
				logDebug("**WARN rB1ExpResult is null for created Parent License " + rNewBeerLicId);
			}
		}
		
		if (rNewWineLicId) {
			var newWineCap = aa.cap.getCap(rNewWineLicId).getOutput();
			aa.cap.runEMSEScriptAfterApplicationSubmit(newWineCap.getCapModel(), rNewWineLicId);
		}
		if (rNewBeerLicId) {
			var newBeerCap = aa.cap.getCap(rNewBeerLicId).getOutput();
			aa.cap.runEMSEScriptAfterApplicationSubmit(newBeerCap.getCapModel(), rNewBeerLicId);
		}
	}

	if (recordSettings.createLP && rNewWineLicId) {
		//create wine LP
		...
		recordSettings.refLPType = "Internet Wine Retailer - " + locField;
		createRefLP4Lookup(rNewWineLicIdString, recordSettings.refLPType, recordSettings.contactType, contactAddressType);

		//Set Business Name and Exp Date
		rNewLP = aa.licenseScript.getRefLicensesProfByLicNbr(aa.serviceProvider, rNewWineLicIdString).getOutput();
		if (rNewLP) {
			rThisLP = rNewLP[0];
			...
			if (rHasParent) {
				aa.licenseScript.associateLpWithCap(rNewWineLicId, rThisLP);
			}

			if (!isEmptyOrNull(rNewWineLicIdString)) {
				...
				if (reqContact != null && reqContact["email"] != null && reqContact["email"] != "") {
					...
					if (thisPublicUser != null) {
						...
						if (!isLicenseConnectedToPublicUser(publicUserSeqNum, rNewWineLicIdString)) {
							associateLPToPublicUser(rNewWineLicIdString, publicUserSeqNum);
	...
	if (recordSettings.createLP && rNewBeerLicId) {
		//create wine LP
		var contactAddressType = handleUndefined(recordSettings.contactAddressType, false);
		if (contactAddressType == "") {
			contactAddressType = null;
		}
		recordSettings.refLPType = "Internet Beer Retailer - " + locField;
		createRefLP4Lookup(rNewBeerLicIdString, recordSettings.refLPType, recordSettings.contactType, contactAddressType);

		//Set Business Name and Exp Date
		rNewLP = aa.licenseScript.getRefLicensesProfByLicNbr(aa.serviceProvider, rNewBeerLicIdString).getOutput();
		if (rNewLP) {
			rThisLP = rNewLP[0];
			rThisLP.setLicenseIssueDate(aa.date.parseDate(dateAdd(aa.util.now(), 0)));
			...
			var editRefResult = aa.licenseScript.editRefLicenseProf(rThisLP);
			if (rHasParent) {
				aa.licenseScript.associateLpWithCap(rNewBeerLicId, rThisLP);
			}

			if (!isEmptyOrNull(rNewBeerLicIdString)) {
				//check if public user exist for the contact type:
				var reqContact = null;
				var contactsList = getContacts();
				if (contactsList && contactsList.length > 0) {
				...
				if (reqContact != null && reqContact["email"] != null && reqContact["email"] != "") {
					...
					if (thisPublicUser != null) {
						var publicUserSeqNum = thisPublicUser.getUserSeqNum();
						if (!isLicenseConnectedToPublicUser(publicUserSeqNum, rNewWineLicIdString)) {
							associateLPToPublicUser(rNewBeerLicIdString, publicUserSeqNum);
						}//user not assoc with LP
					}//thisPublicUser
				}//contact has email
			}//rNewLicIdString is OK
	...
	// Not needed for Internet Retailer
}
...
function licenseIssuance(itemCapId, recordSettings) {
	...
	if (recordSettings.licenseTable != "") {
		var ASITRowsArray = [];
		rLicChildArray = recordSettings.tableLicense.split("/");
		rLicTable = loadASITable(recordSettings.licenseTable);
		for (x in rLicTable) {
			rVehData = rLicTable[x];
			if (recordSettings.childLicense) {
				rNewLicId = createChild(rLicChildArray[0], rLicChildArray[1], rLicChildArray[2], rLicChildArray[3], null, itemCapId);
			} else {
				rNewLicId = createParentLocal(rLicChildArray[0], rLicChildArray[1], rLicChildArray[2], rLicChildArray[3], null);
			}
			var newCap = aa.cap.getCap(rNewLicId).getOutput();
			rNewLicIdString = rNewLicId.getCustomID();
			...
			if (!isEmptyOrNull(recordSettings.columnsMapping)) {
				for (m in recordSettings.columnsMapping) {
					editAppSpecific(recordSettings.columnsMapping[m].customField, rVehData[recordSettings.columnsMapping[m].column], rNewLicId);
				}
			}

			if (recordSettings.mapRow) {
				// Map the current row into the created license table.
				var tbl = new Array();
				tbl.push(rVehData);
				removeASITable(recordSettings.licenseTable, rNewLicId);
				addASITable(recordSettings.licenseTable, tbl, rNewLicId);
			}
			...
			if (rB1ExpResult != null && rB1ExpResult.getB1Expiration() != null) {
				...
				if (recordSettings.expirationType == "Function" && rCustomExpirationFunction != null && rCustomExpirationFunction != "") {
					var dateCalculationFuntion = rCustomExpirationFunction + "(rB1ExpResult, rVehData)";
					var dateResult = eval("(" + dateCalculationFuntion + ")");
					if (dateResult instanceof Date) {
						rB1ExpResult.setExpDate(aa.date.parseDate(dateAdd(dateResult, 0)));
					} else {
			...
			closeTask("Application Status","Issued","","");
			aa.cap.runEMSEScriptAfterApplicationSubmit(newCap.getCapModel(), rNewLicId);
		}
	}
}
